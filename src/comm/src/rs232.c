...
char* comports[RS232_PORTNR] = { "/dev/ttyS0", "/dev/ttyS1", "/dev/ttyS2", "/dev/ttyS3", "/dev/ttyS4", "/dev/ttyS5",
                                ...
                                "/dev/cuaU0", "/dev/cuaU1", "/dev/cuaU2", "/dev/cuaU3", "/dev/motor_kanan", "/dev/motor_kiri", "/dev/motor_belakang",
...
int RS232_OpenComport(int comport_number, int baudrate, const char* mode)
{
    ...
    */
    ...
    // new_port_settings.c_cflag &= ~PARENB;        // Clear parity bit, disabling parity (most common)
    // new_port_settings.c_cflag &= ~CSTOPB;        // Clear stop field, only one stop bit used in communication (most common)
    // new_port_settings.c_cflag &= ~CSIZE;         // Clear all bits that set the data size
    // new_port_settings.c_cflag |= CS8;            // 8 bits per byte (most common)
    // new_port_settings.c_cflag &= ~CRTSCTS;       // Disable RTS/CTS hardware flow control (most common)
    // new_port_settings.c_cflag |= CREAD | CLOCAL; // Turn on READ & ignore ctrl lines (CLOCAL = 1)

    // new_port_settings.c_lflag &= ~ICANON;
    // new_port_settings.c_lflag &= ~ECHO;                                                        // Disable echo
    // new_port_settings.c_lflag &= ~ECHOE;                                                       // Disable erasure
    // new_port_settings.c_lflag &= ~ECHONL;                                                      // Disable new-line echo
    // new_port_settings.c_lflag &= ~ISIG;                                                        // Disable interpretation of INTR, QUIT and SUSP
    // new_port_settings.c_iflag &= ~(IXON | IXOFF | IXANY);                                      // Turn off s/w flow ctrl
    // new_port_settings.c_iflag &= ~(IGNBRK | BRKINT | PARMRK | ISTRIP | INLCR | IGNCR | ICRNL); // Disable any special handling of received bytes

    // new_port_settings.c_oflag &= ~OPOST; // Prevent special interpretation of output bytes (e.g. newline chars)
    // new_port_settings.c_oflag &= ~ONLCR; // Prevent conversion of newline to carriage return/line feed
