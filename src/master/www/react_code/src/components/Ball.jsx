...
export const Ball = () => {
  ...
  useEffect(() => {
    ...
    // Setup the publisher on the '/vision/ball/threshold/params' topic
    const paramsPublisher = new ROSLIB.Topic({
      ros: newRosConn,
      name: "/vision/ball/threshold/params",
      messageType: "std_msgs/UInt8MultiArray",
    });

    const sliderService = new ROSLIB.Service({
      ros: newRosConn,
      name: "/vision/ball/threshold/params/srv",
  ...
  return (
    <>
      <div className="flex justify-center items-center">
        <p className="text-xl">Ball {connectionStat}</p>
      </div>
      <div className="w-full max-h-screen grid grid-cols-4 gap-x-10 px-10 ">
        ...
        <div id="image_placeholder_2" className="w-fit h-fit">
          <img
            src={`http://${window.location.hostname}:9901/stream?topic=/vision/thresholded/ball`}
            alt=""
          />
        </div>
        <div
